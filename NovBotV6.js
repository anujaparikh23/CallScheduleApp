...
var html=df_merged5.toHTML()

session.send('Results are : \n \"%s\"', html);
session.send("<table border=“1” class=“dataframe”> <thead> <tr style=“text-align: right;”> <th></th> <th>Customer_ID</th> <th>Common_Connections</th> <th>Interests</th> <th>Events_Attended</th> <th>Planned_Events</th> </tr> </thead> <tbody> <tr> <th>0</th> <td>011</td> <td>undefined</td> <td>Advance Research in Abdominal Surgery,Cummins Medical Devices,Abbott</td> <td>undefined</td> <td>undefined</td> </tr> <tr> <th>1</th> <td>004</td> <td>Dr. Matt Thomas,Dr. John Yan,Dr. Stuart Cain,Dr. William Robbs,Kim Williams,Peter Dinklage</td> <td>undefined</td> <td>3rd Global Summit on Laser Treatment</td> <td>Private Equity Forum Chicago 2017</td> </tr> <tr> <th>2</th> <td>001</td> <td>Allen Toffel,Dr. Matt Thomas,Dr. Brendan Faser,Dr. Frank Miller,Dr. Sam Fury,Adam Sandler,Dev Patel,Dr. Nikita Romanov</td> <td>Deakin University,Doctors Jobs in US,Pfizer,AstraZeneca</td> <td>NorthEast Summit of Neurologists,World Congress on Innovative Research in Neurology</td> <td>undefined</td> </tr> </tbody> </table>");
...
bot.dialog('CheckPublications', [
    function(session,args) {
        ...
        session.send("|Id|Runbook|Start Time|End Time|Status|<br>|---|---|---|---|---|<br>|Id|Runbook|Start Time|End Time|Status|");

        var name = builder.EntityRecognizer.findEntity(args.intent.entities, 'DoctorName');
        if (name) {
           session.send('Ok! Let me search for Publications of Dr. \'%s\', please give me a moment...', name.entity);
        }
        else
        {
            session.send("I din't understand. Please try again");
        ...
        con.query(query,function (error, results, fields) {
            ...
            if(!results1[0].length)
            {
                 ...
                 for(var i=0;i<results1[0].length;i++){
                         session.send((i+1)+": "+results1[0][i].Individual_Publications);
                 }
            }
            ...
            if(!results1[1].length)
            {
                 ...
                 for(var i=0;i<results1[1].length;i++){
                         session.send((i+1)+": "+results1[0][i].Internal_Sponsored_Trials);
                 }
            }
            ...
            if(!results1[2].length)
            {
                 ...
                 for(var i=0;i<results1[2].length;i++){
                         session.send((i+1)+": "+results1[0][i].Joint_Publications);
                 }
            }
            ...
            if(!results1[3].length)
            {
                 ...
                 for(var i=0;i<results1[3].length;i++){
                         session.send((i+1)+": "+results1[0][i].Third_Party_Trials);
                 }
            } 
